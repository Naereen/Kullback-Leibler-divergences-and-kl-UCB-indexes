...
double _reseqp(int size, double* p, double* V, double klMax, double mV) {
		...
		y += log(u) - klMax;
        // printf("p[0] = %g, V[0] = %g, u = %g, y = %g, tol = %g\n", p[0], V[0], u, fabs(y), tol);
		while (fabs(y)>tol) {
			yp = 0;
			for (i=0; i<size; i++) {
                yp += p[i]/((l-V[i])*(l-V[i]));
...
void _maxEV(int size, double* p, double* V, double klMax, double* Uq) {
	...
	for (i=0; i<size; i++) {
		...
		if (V[i]>M[p[i]>0]) {
			M[p[i]>0] = V[i];
			if(p[i]==0) {
                j = i; // store index of maximum with p[i]==0
            }
		}
		if ((V[i]<m) && (p[i]>0)) {
            m = V[i];
...
static PyMethodDef kullback_leibler_cMethods[] = {
    ...
    {"klucbGauss", klucbGauss, METH_VARARGS, "klucbGauss(x, d, sig2): UCB for Gaussian observations."},
    {"klucbPoisson", klucbPoisson, METH_VARARGS, "klucbPoisson(x, d, precision): UCB for Poisson observations."},
    {"klucbBern", klucbBern, METH_VARARGS, "klucbBern(x, d, precision): UCB for Bernoulli observations."},
    {"klucbExp", klucbExp, METH_VARARGS, "klucbExp(x, d, precision): UCB for Exponential observations."},
    {"klucbGamma", klucbGamma, METH_VARARGS, "klucbGamma(x, d, precision): UCB for Gamma observations."},
...
static struct PyModuleDef kullback_leibler_cModuleDef = {
    PyModuleDef_HEAD_INIT,
    "kullback_leibler_c",  /* name of module */
    "",          /* module documentation, may be NULL */
    -1,          /* size of per-interpreter state of the module, or -1 if the module keeps state in global variables. */
    kullback_leibler_cMethods
};

PyMODINIT_FUNC PyInit_kullback_leibler_c(void) {
    // This is for Python 3. Cf. http://stackoverflow.com/a/28306354/5889533
    return PyModule_Create(&kullback_leibler_cModuleDef);
};
